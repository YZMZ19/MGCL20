...
MGBox& MGBox::operator*= (const MGMatrix& matrix) {
	...
	if(finite()){
		...
		for(int i=0; i<dim; i++)
			m_range[i]=MGInterval(MGINTERVAL_TYPE::MGINTERVAL_INFINITE);
	}
	return *this;
}
...
bool MGBox::crossing2D(const MGStraight& sl)const{
	...
	if(stype== MGSTRAIGHT_TYPE::MGSTRAIGHT_EMPTY) return false;
	...
	MGPosition P0;
	if(stype== MGSTRAIGHT_TYPE::MGSTRAIGHT_UNLIMIT) P0=sl.root_point();
	...
	MGVector v01;
	if(stype== MGSTRAIGHT_TYPE::MGSTRAIGHT_SEGMENT){
