 ...
 *    抽象クラスとなります。Group Elementにより MGObject はGroupの要素とすることができます。
 *    この分類には、MGObject を除くMGGel直下のクラスおよびグループ内のGelの位置を表現する
 ...
 *     ためのクラス MGOfstream , MGIfstream , そしてIGESデータの入出力のクラス
...
enum VIEWMODE{
	DONTCARE=0,
	WIREVIEW=1,		///< wire frame mode
	SHADINGVIEW=2,	///< surface mode
	WIRE_AND_SHADINGVIEW=3, ///< wire and surface mode
	HIGHLIGHT=4
};

///Define  draw element target name.
enum DRAW_TARGET{
	WIRE = 1,
	SHADING = 2,
